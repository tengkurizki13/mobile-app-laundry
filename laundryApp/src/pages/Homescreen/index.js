        ...
        },
        {
          ...
          onPress: () => {
            ...
            dispatch(updateStatusRequestHandler(data, id))
              .then(() => {
                dispatch(fetchRequests());
                setLoading(false); // Mengatur loading menjadi false di sini jika diperlukan
              })
              .then(() => {
                Alert.alert('Pesan', 'Tahap sudah berubah');
              })
              .catch((error) => {
        ...
        },
        {
          ...
          onPress: () => {
            ...
            dispatch(updateStatusRequestHandler(data, id))
              .then(() => {
                setLoading(false); // Mengatur loading menjadi false di sini jika diperlukan
                Alert.alert('Pesan', 'Pembayaran berhasil dilakukan');
              })
              .then(() => {
                dispatch(fetchRequests());
              })
          ...
          onPress: () => {
            ...
            dispatch(deleteRequestHandler(id))
              .then(() => {
                dispatch(fetchRequests())
                  .then(() => {
                    // Menampilkan alert lagi setelah penghapusan berhasil
                    setLoading(false)
                    Alert.alert('Pesan', 'Data telah dihapus.');
                  }).catch((error) =>{
                    console.log("Err",error);
                  })
              })
          },
          style: 'destructive', // menambahkan gaya "destructive"
        },
      ],
      { cancelable: false }
    );
